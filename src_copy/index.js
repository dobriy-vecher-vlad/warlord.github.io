...
import dataUtils from './data/utils.json';
...
import PANEL_utils__1 from './panels/utils/1';
...
const App = withAdaptivity(({ viewWidth }) => {
				...
				if (!reload) {
					...
					if ((dataDonut?.response?.items && dataDonut.response.items.indexOf(dataVK.id) != -1 && dataDonutUser?.response?.items) || dev) {
						...
						if (dataGame?._uid && Number(dataGame?._uid) !== 0) {
							let dataGameItems = await getGame(this.state.server, {
								...
								id: DEFAULT_AUTHORIZATION[this.state.server-1]?.id,
					...
					if (dataGame?._uid && Number(dataGame?._uid) !== 0) {
						...
						let dataGameItems = await getGame(server, {
							...
							id: DEFAULT_AUTHORIZATION[server-1]?.id,
						...
						<SplitCol fixed width="280px" maxWidth="280px">
							<Panel id="navigation">
								<Group>
									...
									<RichCell
										className="RichCell--Context"
										disabled={activeStory === 'utils'}
										data-story="utils"
										onClick={onStoryChange}
										before={<Icon28WrenchOutline />} 
										description="Генераторы, анализы"
										after={isEmbedded&&<Counter size="s" mode="prominent">НОВОЕ</Counter>}
									>Утилиты</RichCell>
					...
					<SplitCol animate={!isDesktop} spaced={isDesktop} width={isDesktop && activeStory ? '560px' : '100%'} maxWidth={isDesktop && activeStory ? '560px' : '100%'}>
						<Epic activeStory={activeStory} tabbar={!isDesktop && activeStory &&
							<Tabbar>
								<TabbarItem
									onClick={onStoryChange}
									selected={['home', 'map', 'bosses', 'arena', 'character', 'guild', 'other', 'utils'].includes(activeStory)}
									data-story="home"
									text="Главная"
							...
							<View id="home" activePanel={!activePanel ? 'home' : activePanel}>
								<Panel id="home">
									...
									<Group separator={isDesktop ? "auto" : "hide"} className="RoundSeparators">
										...
										{isDesktop && <div className="RichCards">
											{dataMain.news.map((item, key) =>
												<RichCard
													key={key}
													title={item.title}
													text={item.description}
													href={item.link}
													avatar={pathImages + item.icon}
												/>
											)}
										</div>}
										{!isDesktop && <React.Fragment>
											...
											<Group>
												<CardGrid className={`CardsNews withIcon CardGrid--xs`}>
													...
													<Card data-story="other" onClick={onStoryChange}><HorizontalCell size='l' header="Разное"><Icon28Cards2Outline style={{['--fill']: 'var(--systemPurple)' }}/></HorizontalCell></Card>
													<Card data-story="utils" onClick={onStoryChange}><HorizontalCell size='l' header="Утилиты"><Icon28WrenchOutline style={{['--fill']: 'var(--systemPink)' }}/></HorizontalCell></Card>
													<Card data-story="rating" onClick={onStoryChange}><HorizontalCell size='l' header="Рейтинг"><Icon28StatisticsOutline style={{['--fill']: 'var(--systemIndigo)' }}/></HorizontalCell></Card>
												</CardGrid>
											...
											<Group>
												...
												<Spacing size={16}/>
												<div className="RichCards" style={{ padding: '0 12px 4px 12px' }}>
													{dataMain.news.map((item, key) =>
														<RichCard
															key={key}
															title={item.title}
								...
								<Panel id="map">
									...
									<Group>
										...
										<div className="RichCards">
											<RichCard
												title="Мустафа"
												text="Предметы и цены"
												avatar={[...Items.filter(item => item.description.toLowerCase().includes('мустаф'))].reverse().slice(0, 4).map(item => pathImages + item.icon)}
												onClick={() => setActivePanel('1', true)}
											/>
											<RichCard
												title="Обыск"
												text="Постройки для обыска"
												avatar={`${pathImages}labels/2b.png`}
												onClick={() => setActivePanel('2', true)}
											/>
											<RichCard
												title="Походы"
												text="Походы и награды"
												avatar={[...dataMap.crusade.points].reverse().slice(0, 4).map(item => pathImages + item.icon)}
												onClick={() => setActivePanel('3', true)}
											/>
											<RichCard
												title="Рейды"
												text="Рейды и награды"
												avatar={[`${pathImages}map/raid_4.png`]}
												onClick={() => setActivePanel('4', true)}
											/>
											<RichCard
												title="Приключения"
												text="Приключения и награды"
												avatar={`${pathImages}labels/5b.png`}
												onClick={() => setActivePanel('5', true)}
											/>
											<RichCard
												title="Районы"
												text="Описания районов"
												avatar={`${pathImages}labels/6b.png`}
												onClick={() => setActivePanel('6', true)}
											/>
											<RichCard
												title="Захват"
												text="Районы для захвата"
												avatar={`${pathImages}labels/7b.png`}
												onClick={() => setActivePanel('7', true)}
											/>
										</div>
									</Group>
									{this.state.snackbar}
							...
							<View id="bosses" activePanel={!activePanel ? 'bosses' : activePanel}>
								<Panel id="bosses">
									...
									<Group>
										...
										<div className="RichCards">
											{isEmbedded&&serverStatus&&<RichCard
												title="Калькулятор"
												text="Затраты на убийство боссов"
												avatar={`${pathImages}labels/26b.png`}
												onClick={() => setActivePanel('1', true)}
											/>}
											<RichCard
												title="Список боссов"
												text="Боссы и награды"
												avatar={`${pathImages}labels/29b.png`}
												onClick={() => setActivePanel('2', true)}
											/>
										</div>
									</Group>
								...
								<Panel id="arena">
									...
									<Group>
										...
										<div className="RichCards">
											{isEmbedded&&serverStatus&&<RichCard
												title="Калькулятор"
												text="Затраты на прохождение арены"
												avatar={`${pathImages}labels/26b.png`}
												onClick={() => setActivePanel('1', true)}
											/>}
											{isEmbedded&&serverStatus&&<RichCard
												title="Анализ"
												text="Анализ сезонов и наград"
												avatar={`${pathImages}labels/27b.png`}
												onClick={() => this.OpenModal('mediaArenaItems')}
											/>}
											<RichCard
												title="Сезоны"
												text="Сезоны и награды"
												avatar={[...Items.filter(item => item.description.toLowerCase().includes('арены'))].reverse().slice(0, 4).map(item => pathImages + item.icon)}
												onClick={() => setActivePanel('2', true)}
											/>
											<RichCard
												title="Лиги"
												text="Лиги и награды"
												avatar={[pathImages + [...dataArena.league].reverse()[0].icon]}
												onClick={() => setActivePanel('3', true)}
											/>
											<RichCard
												title="Сундуки"
												text="Сундуки и награды"
												avatar={[...Items.filter(item => item.description.toLowerCase().includes('сундук'))].reverse().slice(0, 4).map(item => pathImages + item.icon)}
												onClick={() => setActivePanel('4', true)}
											/>
										</div>
									</Group>
								...
								<Panel id="character">
									...
									<Group>
										...
										<div className="RichCards">
											<RichCard
												title="Таланты"
												text="Таланты персонажа"
												avatar={`${pathImages}labels/12b.png`}
												onClick={() => setActivePanel('1', true)}
											/>
											<RichCard
												title="Ресурсы"
												text="Ресурсы и их получение"
												avatar={`${pathImages}labels/14b.png`}
												onClick={() => setActivePanel('3', true)}
											/>
											<RichCard
												title="Питомцы"
												text="Питомцы и награды"
												avatar={[pathImages + [...dataCharacter.pets].reverse()[0].icon]}
												onClick={() => setActivePanel('4', true)}
											/>
											<RichCard
												title="Достижения"
												text="Достижения и их условия"
												avatar={`${pathImages}labels/13b.png`}
												onClick={() => setActivePanel('2', true)}
											/>
											<RichCard
												title="Аватары"
												text="Аватары и их получение"
												avatar={[pathImages + [...dataCharacter.avatars].reverse()[0].icon]}
												onClick={() => setActivePanel('6', true)}
											/>
											<RichCard
												title="Фоны"
												text="Фоны и их получение"
												avatar={pathImages + [...dataCharacter.backgrounds].reverse()[0].icon}
												onClick={() => setActivePanel('5', true)}
											/>
										</div>
									</Group>
								...
								{!isDesktop && <PanelHeader after={!isEmbedded&&this.getCopy(this.state.activeStory)} before={<PanelHeaderBack onClick={() => this.setState({ activeStory: 'home', activePanel: 'home' })}/>}>Гильдия</PanelHeader>}
									<Group>
										...
										<div className="RichCards">
											{isEmbedded&&serverStatus&&<RichCard
												title="Калькулятор"
												text="Затраты на казну гильдии"
												avatar={`${pathImages}labels/26b.png`}
												onClick={() => setActivePanel('1', true)}
											/>}
											<RichCard
												title="Кузница"
												text="Экипировка и цены"
												avatar={[...dataGuild.items].reverse().slice(0, 4).map(item => pathImages + Items[item.id].icon)}
												onClick={() => setActivePanel('3', true)}
											/>
											<RichCard
												title="Набеги"
												text="Набеги и награды"
												avatar={`${pathImages}labels/22b.png`}
												onClick={() => setActivePanel('5', true)}
											/>
											<RichCard
												title="Рейды"
												text="Рейды и награды"
												avatar={pathImages + [...dataGuild.bosses].reverse()[0].icon}
												onClick={() => setActivePanel('6', true)}
											/>
											<RichCard
												title="Улучшения"
												text="Улучшения и их уровни"
												avatar={[pathImages + dataGuild.builds[0].icon]}
												onClick={() => setActivePanel('2', true)}
											/>
											<RichCard
												title="Академия"
												text="Навыки и их уровни"
												avatar={`${pathImages}labels/21b.png`}
												onClick={() => setActivePanel('4', true)}
											/>
										</div>
									</Group>
								...
								{!isDesktop && <PanelHeader after={!isEmbedded&&this.getCopy(this.state.activeStory)} before={<PanelHeaderBack onClick={() => this.setState({ activeStory: 'home', activePanel: 'home' })}/>}>Разное</PanelHeader>}
									<Group>
										...
										<div className="RichCards">
											{isEmbedded&&serverStatus&&<RichCard
												title="Калькулятор"
												text="Затраты на улучшение предмета"
												avatar={`${pathImages}labels/26b.png`}
												onClick={() => setActivePanel('1', true)}
											/>}
											{isEmbedded&&serverStatus&&<RichCard
												title="Новые предметы"
												text="Последние добавленные предметы"
												avatar={[...Items].reverse().slice(0, 4).map(item => pathImages + item.icon)}
												onClick={() => setActivePanel('5', true)}
											/>}
											{isEmbedded&&serverStatus&&<RichCard
												title="Анализ"
												text="Анализ событий и наград"
												avatar={`${pathImages}labels/27b.png`}
												onClick={() => this.OpenModal('mediaEventsItems')}
											/>}
											<RichCard
												title="События"
												text="Ивенты и награды"
												avatar={[...Items.filter(item => item.description.toLowerCase().includes('ивент'))].reverse().slice(0, 4).map(item => pathImages + item.icon)}
												onClick={() => setActivePanel('3', true)}
											/>
											<RichCard
												title="Лотерея"
												text="Награды с лото"
												avatar={[...dataOther.lottery].reverse().slice(0, 4).map(item => pathImages + Items[item.id].icon)}
												onClick={() => setActivePanel('2', true)}
											/>
											<RichCard
												title="Обыск друзей"
												text="Награды с обыска"
												avatar={[...dataOther.search].reverse().slice(0, 4).map(item => pathImages + Items[item.id].icon)}
												onClick={() => setActivePanel('4', true)}
											/>
										</div>
									</Group>
							...
							<View id="utils" activePanel={!activePanel ? 'utils' : activePanel}>
								<Panel id="utils">
								{!isDesktop && <PanelHeader after={!isEmbedded&&this.getCopy(this.state.activeStory)} before={<PanelHeaderBack onClick={() => this.setState({ activeStory: 'home', activePanel: 'home' })}/>}>Утилиты</PanelHeader>}
									<Group>
										{isDesktop && <PanelHeader className='HeaderFix' fixed={false} separator={true} after={this.getCopy(this.state.activeStory)}>Утилиты</PanelHeader>}
										<Gallery bullets={false} showArrows className="Gallery__banners" style={{'--offset': isDesktop?'14px':'24px'}} slideWidth='100%' align="center" timeout="5000">
											{dataUtils.images.map((data, x) => <Banner key={x} className="Gallery__banner --shadow" mode="image" size="m" 
												header="Вспомогательные сервисы"
												subheader="Генераторы, анализы"
												background={<React.Fragment>
													<Spinner size="large" className="Gallery__bannerPreload" />
													<div className="Gallery__bannerImage" style={{backgroundImage: `url(${pathImages}${data})`}}/>
												</React.Fragment>}
										...
										<div className="RichCards">
											{isEmbedded&&serverStatus&&<RichCard
												title="Сезоны"
												text="Анализ сезонов и наград"
												avatar={`${pathImages}labels/32b.png`}
												onClick={() => this.OpenModal('mediaArenaItems')}
											/>}
											{isEmbedded&&serverStatus&&<RichCard
												title="События"
												text="Анализ событий и наград"
												avatar={`${pathImages}labels/28b.png`}
												onClick={() => this.OpenModal('mediaEventsItems')}
											/>}
											<RichCard
												title="Коллекции"
												text="Генератор изображений"
												avatar={[`${pathImages}collections/1205.png`, `${pathImages}collections/868.png`, `${pathImages}collections/442.png`, `${pathImages}collections/1421.png`]}
												onClick={() => setActivePanel('1', true)}
											/>
										</div>
									</Group>
								...
								<PANEL_utils__1 id='1' parent='utils' state={this.state} options={this} />
							</View>
						</Epic>
					</SplitCol>
				</SplitLayout>
	...
	// return <ConfigProvider platform={usePlatform()}><Wiki/></ConfigProvider>;
